...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (document.getElementById('add-products')) {
        document.getElementById('add-products').addEventListener('click', function() {
            var checkboxes = document.querySelectorAll('.products:checked');
            var isValid = true;
            ...
            checkboxes.forEach(function(checkbox) {
                ...
                if (!document.getElementById('sel-product-' + product_id)) {
                    ...
                    if (isValid) {
                        productOptions['total_price'] = parseFloat(productOptions['price']) * parseFloat(productOptions['quantity']);
...
document.addEventListener('click', function(event) {
    ...
    if (event.target.closest('.btn-plus')) {
        ...
        updateButtonState(true, currentValue + 1);
    }
    ...
    if (event.target.closest('.btn-plus-weight')) {
        ...
        updateButtonState(false, currentValue + 1);
    }
    ...
    selected_products.forEach(function(product, index) {
        ...
        if (document.getElementById('order_customer')) {
            for (var key in product) {
                if (product.hasOwnProperty(key) && !['product_id', 'product_name', 'price', 'quantity', 'remark', 'total_price', ''].includes(key)) {
...
function updateButtonState(is_qty, val) {
    const minusButton = document.querySelector(is_qty == true ? '.btn-minus' : '.btn-minus-weight');
    minusButton.disabled = is_qty == true ? (val <= 1) : (val <= 0.1);
}
